...
Renovate still manages direct Cargo.toml version ranges (see renovate.json),
but a raw `cargo update` also resolves transitive dependencies that never
show up as a Cargo.toml change, and Renovate's own lockFileMaintenance
(which used to refresh those) is disabled so its weekly output can share a
single PR with Renovate's regular updates instead of a separate one. This
script is that transitive-only refresh, re-implementing renovate.json's
20-day minimumReleaseAge policy directly against crates.io since that guard
does not apply to a plain lockfile refresh.
"""
